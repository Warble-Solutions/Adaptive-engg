    ...
    <div className="flex flex-col w-full bg-slate-950">
      ...
      <section className="section-hero relative min-h-screen flex items-center px-6 pt-32 pb-20 overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <img src="/images/renewable/ppc-hero-bg.png" alt="PPC Solutions" className="w-full h-full object-cover" />
      ...
      {/* POWER CONTROL TECHNOLOGY IMAGE SECTION */}
      <section className="py-20 bg-slate-50 relative z-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <SectionWrapper>
              <div>
                <span className="text-xs font-bold text-primary uppercase tracking-widest block mb-4">Grid Integration</span>
                <h3 className="text-3xl md:text-4xl font-bold text-slate-900 mb-6 font-heading">Grid-compliant power control logic</h3>
                <p className="text-slate-600 leading-relaxed mb-6">
                  Maintain strict alignment with utility standards and local grid code limits. The Power Plant Controller handles high-speed reactive power management, ramp rate regulation, and voltage correction, coordinating dynamically between inverters and substations.
                </p>
                <div className="flex items-center gap-3 text-sm font-bold text-slate-800">
                  <CheckCircle2 className="text-primary w-5 h-5" />
                  <span>Sub-second control loop execution</span>
                </div>
                <div className="flex items-center gap-3 text-sm font-bold text-slate-800 mt-2">
                  <CheckCircle2 className="text-primary w-5 h-5" />
                  <span>Fully hardware-in-the-loop (HIL) certified</span>
                </div>
              </div>
            </SectionWrapper>
            <SectionWrapper delay={0.2}>
              <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-slate-200">
                <img 
                  ...
                  alt="Substation Transformer and Lines" 
                  className="w-full h-[400px] object-cover hover:scale-105 transition-transform duration-700"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 to-transparent"></div>
                <div className="absolute bottom-6 left-6 text-white">
                  <p className="text-xs font-bold uppercase tracking-widest text-teal-400 mb-1">Substations</p>
                  <h4 className="text-lg font-bold">Utility Grid Interconnection Site</h4>
                </div>
              </div>
            </SectionWrapper>
          </div>
...
function FeatureRow({ title, desc, imageSrc }: { title: string; desc: string; imageSrc?: string }) {
  return (
    <div className="flex items-start gap-5 group p-4 rounded-xl bg-slate-50 transition-colors border border-slate-100 hover:border-primary/50 hover:shadow-xl">
      <div className="w-12 h-12 rounded-lg flex items-center justify-center shrink-0 overflow-hidden bg-white shadow-sm">
        {imageSrc ? (
