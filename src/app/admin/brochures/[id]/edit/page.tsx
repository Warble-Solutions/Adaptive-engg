...
export default function EditBrochurePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/brochures/${id}`, {
        method: "PUT",
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || "Failed to update brochure");
      }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white font-outfit">Edit Brochure</h1>
          <p className="text-gray-500 text-sm mt-0.5">Modify brochure details</p>
        </div>
      </div>
