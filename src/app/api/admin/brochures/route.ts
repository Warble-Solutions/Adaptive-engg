...
export async function GET() {
  ...
  const items = await prisma.brochure.findMany({ orderBy: { sortOrder: "asc" } });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const existing = await prisma.brochure.findUnique({ where: { slug } });
    if (existing) slug = `${slug}-${Date.now()}`;

    const item = await prisma.brochure.create({ data: { ...data, slug } });
